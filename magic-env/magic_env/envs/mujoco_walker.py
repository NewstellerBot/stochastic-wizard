...
# these bounds have only been tested empirically
# pushing them until we get error from mujoco
INERTIA_BOUND = 1e-15
MASS_BOUND = 1e-15
...
class Walker2dEnv(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        # Get magic multiplier from action
        *action, multiplier = action

        # Mask out the multiplier if action is masked
        if self._magic_mask:
            multiplier = 1
