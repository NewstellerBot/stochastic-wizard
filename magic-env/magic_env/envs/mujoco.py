...
class BaseMujocoEnv(gym.Env[np.float64, np.float32]):
    ...
    def _set_action_space(self):
        ...
        # Create actions for controlling the gravity multiplier
        # This action will be used in a multiplicative manner
        # i.e., when gravity = x, and action = a
        #       then gravity = x * a
        #
        # Having a non-zero, positive lower bound, we ensure the agent will
        # never settle on degenerative behaviours.
        low = np.append(low, 1e-1)
        high = np.append(high, 1e2)

        magic_bounds = low[-1], high[-1]

        logger.info("Using magic bounds: [{}, {}]".format(*magic_bounds))
