...
from train.mountaincar.both import STEPS, N_FIGHTERS, N_WIZARDS, ALGO
...
def test_agent(
    ...
    gravity_init = lambda: 0.0025
    gravity_perturbation = lambda: np.random.normal(loc=0, scale=0.001)

    force_init = lambda: 0.001
    force_perturbations = lambda: np.random.normal(loc=0, scale=0.001)
    ...
    env = Monitor(
        gym.make(
            ...
            force_perturbation=force_perturbations,
        )
    )
    ...
    for steps in STEPS:
        agent = ALGO.load(f"./checkpoints/mountaincar/{kind}/{n}/{steps}")
